import type { MatchResult } from "../battle/match-engine";

function compareKeys(left: string, right: string): number {
  return left < right ? -1 : left > right ? 1 : 0;
}

function serializeValue(value: unknown, inArray: boolean): string | undefined {
  if (value === null) {
    return "null";
  }
  ...
  switch (typeof value) {
    ...
    case "object":
      if (Array.isArray(value)) {
        return `[${value
          .map((item) => serializeValue(item, true) ?? "null")
          .join(",")}]`;
      }
...
export function canonicalSerialize(value: unknown): string {
  const serialized = serializeValue(value, false);
...
export function hashMatchResult(result: MatchResult): string {
  return sha256(canonicalSerialize(result));
}
